...
CitibikeTrips.prototype.getToken = function getTokenF(callback) {
    ...
    request(opts, function(error, response, body) {
        if (!error && response.statusCode === 200) {
            ...
            if (response) {
                self.debug && console.error(Date() + ' ' + 'REQ getToken Fail res code:[' + response.statusCode + ']');
            } else {
    ...
    request(opts, function(error, response) {
        if (!error && response.statusCode === 302) {
            ...
            if (response) {
                ...
                self.debug && console.error(Date() + ' ' + 'REQ Login Failed err[' + error + ']');
            }
            return callback(error);
        }
    });
    ...
    request(opts, function(error, response, body) {
        ...
        if (!error && response.statusCode === 200) {
            ...
            $('tr[class=trip]').each(function(i, e) {
                ...
                trip[$(e).attr('id')].retrievedTimestamp = timeNow;
                // manipulate data
